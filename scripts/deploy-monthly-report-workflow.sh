#!/usr/bin/env bash
# ============================================================
# AI Growth System — Deploy "AGS — Monthly Report" n8n workflow
# ------------------------------------------------------------
  ...
  "nodes": [
    ...
    },
    {
      "parameters": {
        ...
        "sendBody": true,
        "specifyBody": "json",
        "jsonBody": "={\n  \"to\": \"{{ \$json.email }}\",\n  \"tenantId\": \"{{ \$json.id }}\",\n  \"subject\": \"Dein monatlicher AGS-Report\",\n  \"html\": \"<!doctype html><html><body style='font-family:Inter,system-ui,sans-serif;background:#0A0F1C;color:#F1F5F9;padding:32px'><div style='max-width:560px;margin:0 auto;background:#111827;border:1px solid #374151;border-radius:16px;padding:32px'><h1 style='margin:0 0 8px;font-size:22px;background:linear-gradient(135deg,#3B82F6,#10B981);-webkit-background-clip:text;-webkit-text-fill-color:transparent'>Dein monatlicher AGS-Report</h1><p style='color:#9CA3AF;margin:0 0 24px'>Hallo {{ \$json.name }}, hier sind deine Wachstums-Zahlen vom letzten Monat.</p><table style='width:100%;border-collapse:collapse'><tr><td style='padding:8px 0;color:#9CA3AF'>Neue Leads</td><td style='padding:8px 0;text-align:right;color:#F1F5F9;font-weight:600'>{{ \$json.metrics.new_leads || 0 }}</td></tr><tr><td style='padding:8px 0;color:#9CA3AF'>Google-Rating</td><td style='padding:8px 0;text-align:right;color:#F1F5F9;font-weight:600'>{{ \$json.metrics.google_rating || '—' }}</td></tr><tr><td style='padding:8px 0;color:#9CA3AF'>Health-Score</td><td style='padding:8px 0;text-align:right;color:#10B981;font-weight:600'>{{ \$json.health_score }}/100</td></tr></table><a href='https://aigrowthsystem.de/dashboard/reports' style='display:inline-block;margin-top:24px;padding:12px 24px;background:linear-gradient(135deg,#3B82F6,#10B981);color:#fff;text-decoration:none;border-radius:10px;font-weight:600'>Vollständigen Report ansehen</a><p style='color:#6B7280;font-size:12px;margin-top:32px'>Automatisch generiert am {{ \$now.format('DD.MM.YYYY') }}</p></div></body></html>\"\n}",
        "options": {
          "timeout": 20000
        }
      },
